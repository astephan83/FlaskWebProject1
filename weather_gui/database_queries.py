    ...
    try:
        ...
        with connection:
            ...
            if id_number != None:
                ...
                statement = "SELECT w.icon FROM weather_icons w WHERE (w.description='"+description+"') AND w.isNight = '"+is_night+"'"
...
def get_saved_cities(uid):
    try:
        ...
        with connection:
            conn = connection.cursor()
            conn.execute("SELECT * FROM settings WHERE uid=?", (uid,))
...
'''
    Saves user's login to database
...
def save_login(username, password):
    try:
        ...
        if connection:
            connection.rollback()
        message = "Error {0}: ".format(e.args[0])
...
'''
    ...
    Parameter:
        username(string): user's name they chose to login with
...
def get_password(username):
    try:
        ...
        with connection:
            conn = connection.cursor()
            conn.execute("SELECT password, uid FROM login WHERE username=?;", (username,))
...
'''
    Updates user login in database
    ...
    Returns:  
        status - if saved successfully or failed
'''
def update_login(username, password, id):
    try:
        connection = sqlt.connect('weather.db')

        with connection:
            conn = connection.cursor()
            conn.execute("UPDATE login SET username=?, password=? WHERE uid=?;", (username, password, id))
            connection.commit()
